...
#make dictionary
fo = open("/home/livefree/Desktop/data.txt","a+")
...
numberOfResults = 1868

content = f.readlines(1040)
...
while i < 500 :
	...
	for c in content :
		...
		print "The overallScore of the essay is (0-5) " + str(overallScore) + "\n"
	...
	#		print key + " " + str(misspeltWordSug[key]) 
		...
		dataToBeWritten = str(numberOfResults) + "\t" + str((1-(float(numMisspelt)/wordCount))*5) + "\t" + str(score) + "\t" + str((grammarCumScore)) + "\t" + str(sentCount) + "\t" + str(beautifulWords) + "\t" + str(format(avgSentLen,'.2f')) + "\n"
		numberOfResults += 1
